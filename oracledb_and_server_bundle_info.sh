...
#		20-10-22	use two brackets for if conditions for more accuracy.
#		20-10-22	Defaulting the script default shell to bash to tolerate if conditions between two brackets.
#
# 
# 
# 
...
col hash for a16
col LIMIT for a30
select u.USERNAME,u.ACCOUNT_STATUS,u.PROFILE,u.DEFAULT_TABLESPACE||' | '||u.TEMPORARY_TABLESPACE "DEFAULT | TEMP TABLESPACE",to_char(u.EXPIRY_DATE,'DD-MON-YY')EXPIRY_DATE,to_char(u.LOCK_DATE,'DD-MON-YY')LOCK_DATE,
...
# If login.sql was renamed during the execution of the script revert it back to its original name:
        if [[ -r ./login.sql_NeutralizedBy${SCRIPT_NAME} ]]
         then
...
if [[ -x ${FILE_NAME} ]]
then
 if [[ -r /etc/rc.d/init.d ]]
  then
   echo ""                                                              >> ${OS_BASELINE}
   echo "SERVICES Settings: chkconfig --list"                           >> ${OS_BASELINE}
   echo "-----------------"                                             >> ${OS_BASELINE}
   /sbin/chkconfig --list 2>/dev/null |sort                             >> ${OS_BASELINE}
   echo ""                                                              >> ${OS_BASELINE}
 fi
fi
...
echo "------------------"                                               >> ${OS_BASELINE}
/sbin/chkconfig --list 2>/dev/null |grep ntp                            >> ${OS_BASELINE}
echo ""                                                                 >> ${OS_BASELINE}
fi
